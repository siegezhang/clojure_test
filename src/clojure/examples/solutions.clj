...
;; (empty? (__ "nothing"))
;; (= (__ "$#A(*&987Zf") "AZ")
(fn e [s] (clojure.string/join "" (filter #(Character/isUpperCase %1) (seq s))))
...
(fn f [col]
  (into [] (java.util.LinkedHashSet. col)))
...
(fn reduce*
  ...
  ([f init [c & cols]]
   (cons init (lazy-seq (when c (reduce* f (f init c) cols))))))
...
(fn prime* [n]
  (let [ no-prime?* (fn [x]
                      (if (loop [i 2 limit (int (Math/sqrt x))]
                        ...
                        nil x))]
    (take n (drop 2 (filter no-prime?* (range))))))
...
(fn map-with* [f & maps]
  (let [to-list (fn [k] (if (coll? k) k (list k)))
    ...
    (apply merge (map map-map* (reduce f* {} maps)))))
...
(fn euler [x]
  (let [gcd* (fn gcd* [a b]
               (cond
                 ...
                 :else a))]
    (if (= x 1) 1
                (count (lazy-seq (filter #(= 1 (gcd* % x)) (range 1 x)))))))
...
(fn anagrams* [words]
  (set
    (filter #(> (count %) 1)
            (map (comp set (partial map second) second) (group-by first (map #(vector (clojure.string/join "" (sort %)) %) words))))))
